...
bool sdl_init() {
 ...
 window = SDL_CreateWindow("SDL Test", VIDEO_WIDTH * VIDEO_SCALE, VIDEO_HEIGHT * VIDEO_SCALE, SDL_WINDOW_ALWAYS_ON_TOP);
 if (!window) {return false;}
 surface_front = SDL_GetWindowSurface(window);
 if (!surface_front) {return false;}
 surface_back = SDL_CreateSurface(VIDEO_WIDTH, VIDEO_HEIGHT, SDL_PIXELFORMAT_INDEX4MSB);
...
void sdl_flip(const u8* data) {
 memcpy(surface_back->pixels, data, VIDEO_WIDTH * VIDEO_HEIGHT / 2);
